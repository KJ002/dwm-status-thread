...
#define status_variables tmlndn
...
void * statusloop(void * dpy_ptr){
        ...
        if (!(dpy = XOpenDisplay(NULL))) {
                fprintf(stderr, (char*)"dwm: cannot open display.\n");
                pthread_exit(NULL);
        }
        ...
        for (;;sleep(60)) {
                ...
                status = smprintf(status_format, status_variables);
...
void startstatusloop(Display * dpy){
        pthread_t s_thread;
        pthread_create(&s_thread, NULL, &statusloop, (void*)dpy);
}
